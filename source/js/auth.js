...
(function () {
  ...
  async function seedDefaults() {
    ...
    // 데모(미연결) 모드 한정 — 심사자가 바로 둘러볼 수 있도록 샘플 학생 시드.
    //  · 블라인드 준수: 식별 불가한 순우리말 이름만 사용(성씨·실명 없음)
    //  · 실제 Firebase 운영 시에는 생성하지 않음
    if (!useFB()) {
      const BADGE_T = [[0, '새싹'], [20, '탐구러'], [50, '질문왕'], [100, '수학멘토'], [200, '수학마스터']];
      const badgesFor = (p) => BADGE_T.filter(([t]) => p >= t).map(([, n]) => n);
      const sample = [
        { no: 1, name: '가온', points: 140, comp: { problem: 82, reason: 75, comm: 68, connect: 71, info: 88 }, quiz: 90, visits: 23 },
        { no: 2, name: '나래', points: 60, comp: { problem: 64, reason: 58, comm: 72, connect: 55, info: 61 }, quiz: 70, visits: 12 },
        { no: 3, name: '다온', points: 210, comp: { problem: 91, reason: 88, comm: 80, connect: 85, info: 93 }, quiz: 95, visits: 31 },
        { no: 4, name: '라온', points: 25, comp: { problem: 48, reason: 52, comm: 60, connect: 45, info: 50 }, quiz: 55, visits: 8 },
        { no: 5, name: '마루', points: 95, comp: { problem: 73, reason: 70, comm: 66, connect: 78, info: 74 }, quiz: 80, visits: 17 }
      ];
      for (const s of sample) {
        const loginId = `107${pad2(s.no)}`;            // 1학년 7반 N번
        const uid = makeUid('student', loginId);
        if (await Store.getUser(uid)) continue;
        await Store.saveUser({
          uid, role: 'student', loginId, name: s.name,
          grade: 1, classNo: 7, studentNo: s.no, classId: '107',
          pwHash: hash(loginId), mustChangePw: false, createdAt: Date.now(),
          competency: s.comp, points: s.points, badges: badgesFor(s.points),
          visits: s.visits, lastSeenAt: Date.now() - s.no * 36e5,
          reviewQuiz: { lastDate: '2026-06-30', lastScore: s.quiz, lastAt: Date.now() - s.no * 72e5 }
        });
      }
    }
  }
