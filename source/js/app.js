...
(function () {
  ...
  function showLogin() {
    ...
    if (window.Portfolio) Portfolio.teardown();  // 포트폴리오 차트 정리
    startScreen.classList.add('hidden');
  ...
  function openLangModal() {
    ...
    const box = openModal(`
      <div class="flex items-center justify-between mb-1">
        <h3 class="text-lg font-bold text-slate-800">🌐 언어 설정</h3>
        <button class="modal-close text-slate-400 hover:text-slate-700 text-xl leading-none">✕</button>
      </div>
      <p class="text-sm text-slate-500 mb-3">사이드바·헤더 등 화면 언어를 바꿉니다. (문항·자료 등 입력 내용은 원문 유지)</p>
      <div class="i18n-list">
        ${I18N.LANGS.map((l) => `<button class="i18n-opt ${l.code === cur ? 'on' : ''}" data-l="${l.code}">${l.flag} ${l.name}</button>`).join('')}
      </div>
    `);
    box.querySelector('.modal-close').addEventListener('click', closeModal);
    box.querySelectorAll('.i18n-opt').forEach((b) =>
      b.addEventListener('click', () => {
        I18N.set(b.dataset.l);
        box.querySelectorAll('.i18n-opt').forEach((x) => x.classList.toggle('on', x === b));
      }));
  }
  ...
  function navigate(viewKey) {
    ...
    } else if (viewKey === 'portfolio' && window.Portfolio) {
      Portfolio.render(viewContainer, Auth.user);
    } else if (viewKey === 'etc' && window.Etc) {
      Etc.render(viewContainer, Auth.user);
    } else {
  ...
  /* ── 탭3: AI(Claude) 키 설정 — 교사 계정에 저장, 소스 미포함 ──
     ※ 입력칸에 기존 키를 점(•)으로 미리 채우지 않는다.
        (미리 채우면 사용자가 점 문자열을 그대로 저장해 키가 깨지고 401이 발생했음) */
  async function renderAiTab(body) {
    ...
    body.innerHTML = `
      ...
      <input id="api-key" type="password" class="form-input" placeholder="${key ? '변경하려면 새 키 입력 (sk-ant-...)' : 'sk-ant-...'}" autocomplete="off" spellcheck="false">
      <p class="text-xs text-slate-400 mt-1">
        ${key
          ? `현재 등록된 키: <b class="font-mono text-slate-600">${maskKey(key)}</b> · 변경하려면 <b>새 키 전체</b>를 붙여넣어 저장하세요.`
          : '아직 등록된 키가 없습니다. <b>sk-ant-</b> 로 시작하는 키를 붙여넣으세요.'}
      </p>
      <div class="flex justify-end gap-2 mt-4">
        ${key ? '<button id="del-key" class="btn-ghost">키 삭제</button>' : ''}
        <button id="save-key" class="btn-primary">저장 후 확인</button>
      </div>
      <p id="api-msg" class="text-sm text-right mt-2 min-h-[1.25rem]"></p>
    `;
    const input = body.querySelector('#api-key');
    const msg = body.querySelector('#api-msg');
    const setMsg = (text, color) => { msg.textContent = text; msg.className = `text-sm text-right mt-2 min-h-[1.25rem] ${color || 'text-slate-500'}`; };

    body.querySelector('#save-key').addEventListener('click', async () => {
      const v = input.value.trim();
      if (!v) { setMsg('새 키를 입력하세요.', 'text-amber-600'); return; }
      // 깨진 값(점 문자 등) 방지
      if (/^[•·*\s]+$/.test(v)) { setMsg('키가 올바르지 않습니다. 실제 키(sk-ant-...)를 붙여넣으세요.', 'text-red-500'); return; }
      if (!v.startsWith('sk-ant-') && !confirm('이 키는 sk-ant- 로 시작하지 않습니다. Claude(Anthropic) 키가 맞는지 확인하세요.\n그래도 저장할까요?')) return;

      setMsg('저장하고 키를 확인하는 중…', 'text-violet-600');
      await Auth.setApiKey(v);
      const ok = await verifyApiKey(v);
      if (ok === true) { setMsg('✅ 저장되었습니다. 키가 정상 확인되었습니다.', 'text-green-600'); renderAiTab(body); }
      else if (ok === false) { setMsg('⚠ 저장했지만 Anthropic이 키를 거부했습니다(401). 키를 다시 확인하세요.', 'text-red-500'); }
      else { setMsg('✅ 저장되었습니다. (네트워크 문제로 즉시 확인은 건너뜀)', 'text-green-600'); renderAiTab(body); }
    });

    const delBtn = body.querySelector('#del-key');
  ...
  // 키 일부만 표시 (앞 prefix + 끝 4자리)
  function maskKey(k) {
    k = String(k || '');
    if (k.length <= 10) return '••••';
    const head = k.startsWith('sk-ant-') ? 'sk-ant-' : k.slice(0, 4);
    return `${head}…${k.slice(-4)}`;
  }

  // Anthropic에 가벼운 호출로 키 유효성 확인 → true(정상)/false(거부)/null(확인불가)
  async function verifyApiKey(key) {
    try {
      ...
      return null; // 그 외 상태(429 등)는 키 인증 자체는 통과로 보고 확인불가 처리
    } catch (e) {
      return null; // 네트워크/CORS 등 → 확인 건너뜀
    }
  }
