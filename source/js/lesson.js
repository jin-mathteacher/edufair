...
(function () {
  ...
  const TABS = [
    ...
    { key: 'collab',   label: '협업의 장',     icon: '🤝', step: null },     // 구현됨
    { key: 'homework', label: '과제방',        icon: '✏️', step: 'STEP 09' },
    { key: 'material', label: '자료방',        icon: '📂', step: null },     // 구현됨
    { key: 'selfeval', label: '자기평가 보고서', icon: '📝', step: null }      // 구현됨(STEP 07 요청)
  ];
  ...
  let currentSubject = null;   // 선택된 과목 { id, name, classId }
  let subjectsUnsub = null;    // 과목 목록 구독 해제
  let selfEvalUnsub = null;    // 자기평가 보고서 구독 해제

  function cleanup() {
    if (matUnsub) { try { matUnsub(); } catch (e) {} matUnsub = null; }
    if (lessonsUnsub) { try { lessonsUnsub(); } catch (e) {} lessonsUnsub = null; }
    if (subjectsUnsub) { try { subjectsUnsub(); } catch (e) {} subjectsUnsub = null; }
    if (selfEvalUnsub) { try { selfEvalUnsub(); } catch (e) {} selfEvalUnsub = null; }
    if (window.Collab) Collab.teardown();   // 협업의 장 실시간 구독·타이머 정리
    closeViewer();             // 열려있던 슬라이드 뷰어 정리
  }

  // 학습실 진입 → 항상 '과목 선택' 화면부터
  async function render(container, currentUser) {
    cleanup();
    user = currentUser;
    rootEl = container;
    currentSubject = null;
    await renderSubjectLanding(container);
  }

  /* ── 과목 선택/만들기 (학습실 첫 화면) ── */
  async function renderSubjectLanding(container) {
      ...
      <div class="lesson-wrap">
        <div class="card">
          <div class="collab-head">
            <div>
              <h3 class="dash-title" style="margin:0">📚 학습실 — 과목 선택</h3>
              <p class="collab-sub">${isTeacher
                ? '과목을 만들고 반을 지정한 뒤, 과목을 선택하면 그 과목의 수업·활동이 보입니다.'
                : '과목을 선택하면 우리 반 수업·활동을 볼 수 있어요.'}</p>
            </div>
            ${isTeacher ? '<button id="subj-add" class="btn-primary">＋ 과목 만들기</button>' : ''}
          </div>
          <div id="subj-list" class="subj-grid"><p class="collab-loading">불러오는 중…</p></div>
        </div>
      </div>`;
    if (isTeacher) container.querySelector('#subj-add').addEventListener('click', openSubjectCreate);

    const draw = (map) => {
      const el = container.querySelector('#subj-list');
      if (!el) return;
      let subs = Object.entries(map || {}).map(([id, s]) => ({ id, ...s }));
      if (!isTeacher) subs = subs.filter((s) => s.classId === user.classId);
      subs.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
      if (!subs.length) {
        el.innerHTML = `<p class="collab-empty">${isTeacher
          ? '아직 만든 과목이 없습니다. ＋ 과목 만들기로 시작하세요.'
          : '아직 우리 반 과목이 없습니다. 선생님이 과목을 만들면 표시됩니다.'}</p>`;
        return;
      }
      el.innerHTML = subs.map((s) => `
        <div class="subj-card" data-id="${s.id}">
          <div class="subj-ic">📘</div>
          <div class="subj-info"><p class="subj-name">${esc(s.name)}</p><p class="subj-meta">${esc(cidLabel(s.classId))}</p></div>
          ${isTeacher ? '<button class="btn-mini subj-del text-red-500">삭제</button>' : ''}
        </div>`).join('');
      el.querySelectorAll('.subj-card').forEach((card) => {
        const id = card.dataset.id; const s = map[id];
        card.addEventListener('click', (e) => { if (e.target.closest('.subj-del')) return; enterSubject({ id, name: s.name, classId: s.classId }); });
        const del = card.querySelector('.subj-del');
        if (del) del.addEventListener('click', async () => {
          if (!confirm('이 과목을 삭제할까요? (수업·자료 자체는 남습니다)')) return;
          await DB.remove(`subjects/${id}`);
          if (!useFB()) draw((await DB.read('subjects')) || {});
        });
      });
    };
    const un = DB.subscribe('subjects', draw);
    if (un) subjectsUnsub = un; else draw((await DB.read('subjects')) || {});
  }

  async function openSubjectCreate() {
    const classes = await getClasses();
    if (!classes.length) { alert('먼저 ⚙️ 설정에서 학생을 등록해 반을 만들어 주세요.'); return; }
    const box = openModal(`
      <h3 class="text-lg font-bold text-slate-800 mb-3">과목 만들기</h3>
      <form id="subj-form" class="space-y-3" autocomplete="off">
        <div><label class="form-label">과목 이름</label>
          <input id="subj-name" class="form-input" placeholder="예) 수학, 인공지능 수학, 미적분" required></div>
        <div><label class="form-label">대상 반</label>
          <select id="subj-class" class="form-input">
            ${classes.map((c) => `<option value="${c.classId}" data-g="${c.grade}" data-c="${c.classNo}">${esc(classLabel(c.grade, c.classNo))} (${c.students.length}명)</option>`).join('')}
          </select></div>
        <p id="subj-err" class="text-red-500 text-sm min-h-[1.1rem]"></p>
    ...
    box.querySelector('#subj-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const name = box.querySelector('#subj-name').value.trim();
      if (!name) { box.querySelector('#subj-err').textContent = '과목 이름을 입력하세요.'; return; }
      const opt = box.querySelector('#subj-class').selectedOptions[0];
      const sid = genId();
      await DB.write(`subjects/${sid}`, {
        id: sid, name, classId: opt.value, grade: +opt.dataset.g, classNo: +opt.dataset.c,
        createdBy: user.uid, createdByName: user.name || user.loginId, createdAt: Date.now()
      });
      closeModal(); toast('과목을 만들었습니다.');
      if (!useFB()) renderSubjectLanding(rootEl);
    });
  }
  ...
  function backToSubjects() {
    ...
    if (lessonsUnsub) { try { lessonsUnsub(); } catch (e) {} lessonsUnsub = null; }
    if (selfEvalUnsub) { try { selfEvalUnsub(); } catch (e) {} selfEvalUnsub = null; }
    if (window.Collab) Collab.teardown();
    closeViewer();
    currentSubject = null;
    renderSubjectLanding(rootEl);
  }
  ...
  function renderShell(container) {
    container.innerHTML = `
      <div class="lesson-wrap">
        <div class="subj-bar">
          <button id="subj-back" class="btn-ghost">← 과목 목록</button>
          <span class="subj-current">📘 ${esc(currentSubject.name)} <span class="subj-current-meta">${esc(cidLabel(currentSubject.classId))}</span></span>
        </div>
    ...
    container.querySelector('#subj-back').addEventListener('click', backToSubjects);
    container.querySelector('#lesson-tabs').addEventListener('click', (e) => {
      const t = e.target.closest('.lesson-tab'); if (!t) return; selectTab(t.dataset.tab);
    });
    selectTab(activeTab);
  }
  ...
  function selectTab(key) {
    ...
    if (lessonsUnsub) { try { lessonsUnsub(); } catch (e) {} lessonsUnsub = null; }
    if (selfEvalUnsub) { try { selfEvalUnsub(); } catch (e) {} selfEvalUnsub = null; }
    if (window.Collab) Collab.teardown();   // 협업의 장 정리
    ...
    else if (key === 'collab' && window.Collab) Collab.render(body, user);
    else if (key === 'selfeval') renderSelfEval(body);
  ...
  /* ============================================================
     2-B) 자기평가 보고서 (교사 작성·활성화 → 학생 작성/제출)
       · /selfEval/{subjectId}          = { title, guide, items[], active, updatedByName, updatedAt }
       · /selfEvalSub/{subjectId}/{uid} = { name, ratings{}, comments{}, overall, submittedAt }
  ============================================================ */
  const DEFAULT_SEVAL = () => ({
    title: '자기평가 보고서', guide: '이번 과목 학습을 스스로 돌아보고 솔직하게 작성해 보세요.',
    items: ['이번 과목에서 가장 성장했다고 느낀 점은 무엇인가요?', '아쉬웠던 점과 앞으로의 개선 계획은 무엇인가요?', '수업·과제·협업에 참여한 나의 태도를 스스로 평가해 보세요.'],
    active: false
  });

  async function renderSelfEval(body) {
    const sid = currentSubject && currentSubject.id;
    if (!sid) { body.innerHTML = '<p class="mat-empty">과목을 먼저 선택하세요.</p>'; return; }
    const isTeacher = user.role === 'teacher';
    body.innerHTML = '<div class="card"><p class="mat-loading">불러오는 중…</p></div>';
    const report = (await DB.read(`selfEval/${sid}`)) || DEFAULT_SEVAL();

    if (isTeacher) return renderSelfEvalTeacher(body, sid, report);
    return renderSelfEvalStudent(body, sid, report);
  }

  function renderSelfEvalTeacher(body, sid, report) {
    const paint = () => {
      if (selfEvalUnsub) { try { selfEvalUnsub(); } catch (e) {} selfEvalUnsub = null; } // 재렌더 시 이전 구독 해제
      body.innerHTML = `
        <div class="card">
          <div class="collab-head">
            <div><h3 class="dash-title" style="margin:0">📝 자기평가 보고서 — ${esc(currentSubject.name)}</h3>
              <p class="collab-sub">보고서를 구성하고 <b>활성화</b>하면 학생 화면에서 작성할 수 있습니다.</p></div>
            <label class="seval-toggle"><input type="checkbox" id="seval-active" ${report.active ? 'checked' : ''}> <span>학생에게 활성화</span></label>
          </div>
          <label class="form-label">보고서 제목</label>
          <input id="seval-title" class="form-input" value="${esc(report.title || '')}">
          <label class="form-label mt-2">안내문</label>
          <textarea id="seval-guide" class="form-input" rows="2">${esc(report.guide || '')}</textarea>
          <label class="form-label mt-2">자기평가 항목 <span class="text-slate-400 font-normal">(각 항목마다 학생이 1~5점 자기평가 + 서술)</span></label>
          <div id="seval-items" class="seval-items">
            ${(report.items || []).map((it, i) => `
              <div class="seval-item-edit" data-i="${i}">
                <span class="seval-idx">${i + 1}</span>
                <input class="form-input seval-itxt" value="${esc(it)}" placeholder="항목 ${i + 1}">
                <button type="button" class="btn-mini seval-idel text-red-500">✕</button>
              </div>`).join('')}
          </div>
          <button type="button" class="btn-mini mt-1" id="seval-iadd">＋ 항목 추가</button>
          <div class="flex gap-2 justify-end mt-3">
            <button id="seval-save" class="btn-primary">저장</button>
          </div>
          <p id="seval-msg" class="text-sm text-right mt-1 min-h-[1.1rem]"></p>
          <div id="seval-subs" class="mt-4"></div>
        </div>`;

      const collect = () => ({
        title: body.querySelector('#seval-title').value.trim() || '자기평가 보고서',
        guide: body.querySelector('#seval-guide').value.trim(),
        items: [...body.querySelectorAll('.seval-itxt')].map((t) => t.value.trim()).filter(Boolean),
        active: body.querySelector('#seval-active').checked
      });
      body.querySelector('#seval-iadd').addEventListener('click', () => { const r = collect(); r.items.push(''); Object.assign(report, r); paint(); });
      body.querySelectorAll('.seval-idel').forEach((b) => b.addEventListener('click', (e) => {
        const r = collect(); r.items.splice(+e.target.closest('.seval-item-edit').dataset.i, 1); Object.assign(report, r); paint();
      }));
      const save = async () => {
        const r = collect();
        if (!r.items.length) { const m = body.querySelector('#seval-msg'); m.className = 'text-sm text-right mt-1 text-amber-600'; m.textContent = '항목을 1개 이상 입력하세요.'; return r; }
        await DB.write(`selfEval/${sid}`, Object.assign({ updatedByName: user.name || user.loginId, updatedAt: Date.now() }, r));
        Object.assign(report, r);
        const m = body.querySelector('#seval-msg'); m.className = 'text-sm text-right mt-1 text-green-600'; m.textContent = '✅ 저장되었습니다.' + (r.active ? ' (학생에게 활성화됨)' : '');
        return r;
      };
      body.querySelector('#seval-save').addEventListener('click', save);
      body.querySelector('#seval-active').addEventListener('change', save); // 토글 즉시 반영

      // 제출 현황
      const subsEl = body.querySelector('#seval-subs');
      const drawSubs = (map) => {
        const subs = Object.values(map || {});
        subsEl.innerHTML = `<h4 class="dash-title" style="margin:6px 0 8px">제출 현황 (${subs.length}명)</h4>` +
          (subs.length ? `<div class="seval-sub-list">${subs.sort((a, b) => (b.submittedAt || 0) - (a.submittedAt || 0)).map((s) => `
            <button class="seval-sub" data-uid="${esc(s.uid || '')}"><b>${esc(s.name || '학생')}</b> <span class="text-xs text-slate-400">평균 ${avgRating(s)}점</span></button>`).join('')}</div>`
            : '<p class="text-sm text-slate-400">아직 제출한 학생이 없습니다.</p>');
        subsEl.querySelectorAll('.seval-sub').forEach((b) => b.addEventListener('click', () => {
          const s = subs.find((x) => (x.uid || '') === b.dataset.uid) || subs[0];
          openSevalDetail(report, s);
        }));
      };
      const un = DB.subscribe(`selfEvalSub/${sid}`, (m) => { const arr = {}; Object.entries(m || {}).forEach(([uid, v]) => arr[uid] = Object.assign({ uid }, v)); drawSubs(arr); });
      if (un) selfEvalUnsub = un; else DB.read(`selfEvalSub/${sid}`).then((m) => { const arr = {}; Object.entries(m || {}).forEach(([uid, v]) => arr[uid] = Object.assign({ uid }, v)); drawSubs(arr); });
    };
    paint();
  }

  const avgRating = (s) => { const v = Object.values((s && s.ratings) || {}); return v.length ? (v.reduce((a, b) => a + (+b || 0), 0) / v.length).toFixed(1) : '-'; };
  function openSevalDetail(report, s) {
    const box = openModal(`
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-lg font-bold text-slate-800">${esc(s.name || '학생')} — 자기평가</h3>
        <button class="modal-close text-slate-400 hover:text-slate-700 text-xl leading-none">✕</button>
      </div>
      <div class="seval-detail">
        ${(report.items || []).map((it, i) => `
          <div class="seval-d-item"><p class="seval-d-q">${i + 1}. ${esc(it)}</p>
            <p class="seval-d-r">자기평가 <b>${s.ratings && s.ratings[i] != null ? s.ratings[i] : '-'}</b>/5</p>
            <p class="seval-d-c">${esc((s.comments && s.comments[i]) || '(서술 없음)')}</p></div>`).join('')}
        ${s.overall ? `<div class="seval-d-item"><p class="seval-d-q">총평</p><p class="seval-d-c">${esc(s.overall)}</p></div>` : ''}
      </div>`);
    box.classList.add('modal-wide');
    box.querySelector('.modal-close').addEventListener('click', closeModal);
  }

  async function renderSelfEvalStudent(body, sid, report) {
    if (!report || !report.active) {
      body.innerHTML = `<div class="card text-center py-14"><div class="text-5xl mb-3">📝</div>
        <p class="text-slate-600">아직 자기평가 보고서가 열리지 않았습니다.</p>
        <p class="text-sm text-slate-400 mt-1">선생님이 활성화하면 여기에서 작성할 수 있어요.</p></div>`;
      return;
    }
    const prev = (await DB.read(`selfEvalSub/${sid}/${user.uid}`)) || null;
    body.innerHTML = `
      <div class="card">
        <h3 class="dash-title" style="margin:0 0 4px">📝 ${esc(report.title)}</h3>
        ${report.guide ? `<p class="collab-sub" style="margin-bottom:12px">${esc(report.guide)}</p>` : ''}
        <div class="seval-form">
          ${(report.items || []).map((it, i) => `
            <div class="seval-q">
              <p class="seval-q-t">${i + 1}. ${esc(it)}</p>
              <div class="seval-rate" data-i="${i}">
                ${[1, 2, 3, 4, 5].map((n) => `<label class="seval-star"><input type="radio" name="sr-${i}" value="${n}" ${prev && prev.ratings && +prev.ratings[i] === n ? 'checked' : ''}> ${n}</label>`).join('')}
                <span class="seval-rate-hint">1 매우부족 ~ 5 매우잘함</span>
              </div>
              <textarea class="form-input seval-c" data-i="${i}" rows="2" placeholder="이유·근거를 서술하세요">${esc(prev && prev.comments ? (prev.comments[i] || '') : '')}</textarea>
            </div>`).join('')}
          <label class="form-label mt-2">총평 (선택)</label>
          <textarea id="seval-overall" class="form-input" rows="3" placeholder="이번 과목 전체에 대한 나의 소감·다짐">${esc(prev ? (prev.overall || '') : '')}</textarea>
          <div class="flex gap-2 justify-end mt-3">
            <button id="seval-submit" class="btn-primary">${prev ? '다시 제출' : '제출하기'}</button>
          </div>
          <p id="seval-smsg" class="text-sm text-right mt-1 min-h-[1.1rem]">${prev ? '이미 제출했습니다. 수정 후 다시 제출할 수 있어요.' : ''}</p>
        </div>
      </div>`;
    body.querySelector('#seval-submit').addEventListener('click', async () => {
      const ratings = {}, comments = {};
      (report.items || []).forEach((it, i) => {
        const r = body.querySelector(`input[name="sr-${i}"]:checked`); if (r) ratings[i] = +r.value;
        const c = body.querySelector(`.seval-c[data-i="${i}"]`); if (c && c.value.trim()) comments[i] = c.value.trim();
      });
      const overall = body.querySelector('#seval-overall').value.trim();
      if (!Object.keys(ratings).length && !overall) { const m = body.querySelector('#seval-smsg'); m.className = 'text-sm text-right mt-1 text-amber-600'; m.textContent = '자기평가를 1개 이상 선택하거나 총평을 작성하세요.'; return; }
      await DB.write(`selfEvalSub/${sid}/${user.uid}`, { name: user.name || user.loginId, classId: user.classId || '', ratings, comments, overall, submittedAt: Date.now() });
      const m = body.querySelector('#seval-smsg'); m.className = 'text-sm text-right mt-1 text-green-600'; m.textContent = '✅ 제출되었습니다. 선생님이 확인할 수 있어요.';
    });
  }
  ...
  async function loadLessons() {
    ...
    let items = Object.values(obj).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
    // 선택된 과목의 수업만 (새 수업은 subjectId 보유 / 과거 수업은 같은 반이면 표시)
    if (currentSubject) items = items.filter((l) => l.subjectId === currentSubject.id || (!l.subjectId && l.classId === currentSubject.classId));
  ...
  async function openCreateModal() {
    if (user.role !== 'teacher') return;
    if (!currentSubject) { alert('먼저 과목을 선택하세요.'); return; }
    const box = openModal(`
      <h3 class="text-lg font-bold text-slate-800 mb-1">새 수업 만들기</h3>
      <p class="text-xs text-slate-500 mb-4">과목 <b>${esc(currentSubject.name)}</b> · ${esc(cidLabel(currentSubject.classId))}</p>
